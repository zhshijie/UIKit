...
- (void)viewDidLoad
{
    ...
    _scroll.frame =CGRectMake(0,30, 320, 100);
    _scroll.showsHorizontalScrollIndicator = NO;
    _scroll.showsVerticalScrollIndicator = NO;
    _scroll.pagingEnabled = NO;
    
    //加载显示屏,并为显示屏增加滑动视图
    _screen = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, _screen_width, 100)];
    _screen.text =@"0";
    _screen.textAlignment = NSTextAlignmentRight;
    _screen.font = [UIFont fontWithName:nil size:50];
    _screen.textColor = [UIColor whiteColor];
    _screen.adjustsFontSizeToFitWidth = YES;
    _screen.minimumScaleFactor = 20;
    
    _scroll.contentSize =CGSizeMake(_screen_width, 100);
    _scroll.contentOffset = CGPointMake(_screen_width-320,0);
    [_scroll addSubview:_screen];
 ...
 */
int screenflag = 0;
int numbers = 0;//输入数字的最大长度

int numOfcal = 0;//判断是否连续输入运算符号
static int _doule = 1;//按删除键的次数
...
-(void)isequalAction:(UIButton *)sender
{
    ...
    if([sender.titleLabel.text isEqualToString:@"="])
    {
        ...
        while([new length]*30>_screen_width) {
            _screen_width+=320;
        }
        [self loadText];
        _screen.text = new;
...
-(void)loadText
{
    _scroll.contentSize =CGSizeMake(_screen_width, 100);
    _scroll.contentOffset = CGPointMake(_screen_width-320,0);
    _screen.frame = CGRectMake(0, 0, _screen_width, 100);
}
...
-(void)backNum:(UISwipeGestureRecognizer*)swipeGeser
{
    ...
    //如果删除的最后一个字符是小数点，flag＝0；
    NSString *value = [string substringFromIndex:[string length]-1];
